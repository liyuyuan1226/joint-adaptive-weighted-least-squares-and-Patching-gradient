...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	//自加�?	
	plhs[1] = mxCreateSparse(m*n, m*n, winSizeSmooth*winSizeSmooth*m*n, mxREAL); 
    double *WeightColorPr2 = mxGetPr(plhs[1]); // the output depth weight  sparse matrix for data term
    mwIndex *WeightColorIr2 = mxGetIr(plhs[1]);
    mwIndex *WeightColorJc2 = mxGetJc(plhs[1]);
	//over
    ...
    for(j=0;j<n;j++)
    {
        for(i=0;i<m;i++)
        {
            ...
            for(t=0;t<winSizeSmooth;t++)
            {
                for(s=0;s<winSizeSmooth;s++)
                {
                    ...
                    { 
						...
						diffmax = max(diffR,diffG,diffB);
                        
                        WeightColorPr[counter] = ColorWeightLUT[int(diffR*diffR + diffG*diffG + diffB*diffB)];
						WeightColorPr2[counter] = ColorWeightLUT2[int(diffmax*diffmax)];//自加
						...
						WeightColorIr2[counter] = (mwIndex)Coor[t*winSizeSmooth + s];//自加
			...
			WeightColorJc2[j*m + i + 1] = counter;//自加
        }
